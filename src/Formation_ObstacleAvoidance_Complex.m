...
for k=1:max_iter
    ...
    for i=1:swarm_size
        ...
        for j=[1:(i-1),(i+1):swarm_size]
            ...
            if k >= 25
                for idx = 1:swarm_size
                    % Calculate move_to_goal vector for each robot
                    V_move_to_goal = move_to_goal_vector(x_i(idx), y_i(idx), x_t, y_t);
            
                    % Calculate nearest obstacle and distance
                    [x_o, y_o, d_0] = nearest_obstacle(x_i(idx), y_i(idx), obstacles);
            
                    % Calculate avoid_obstacle vector for each robot
                    V_avoid_obstacle = avoid_obstacle_vector(x_i(idx), y_i(idx), x_o, y_o, V_move_to_goal);
            
                    % Calculate follow_wall vector
                    V_follow_wall = follow_wall_vector(x_i(idx), y_i(idx), x_o, y_o);
