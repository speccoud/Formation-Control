...
% Define the Jam Area square
jamCenterX = 100;  % X-coordinate of the center
jamCenterY = 10;  % Y-coordinate of the center
jamSideLength = 25;  % Side length of the square
...
for k=1:max_iter
    ...
    alphaValue = 0.4;  % Transparency level (0: fully transparent, 1: fully opaque)
    
    % Calculate the coordinates of the square's corners
    x = [jamCenterX - jamSideLength/2, jamCenterX + jamSideLength/2, jamCenterX + jamSideLength/2, jamCenterX - jamSideLength/2];
    y = [jamCenterY - jamSideLength/2, jamCenterY - jamSideLength/2, jamCenterY + jamSideLength/2, jamCenterY + jamSideLength/2];

    % Add a partially transparent square to the figure
    patch(x, y, 'r', 'FaceAlpha', alphaValue, 'EdgeColor', 'red');
    ...
    for i=1:swarm_size
        ...
        for j=[1:(i-1),(i+1):swarm_size]
            ...
            if lineCrossesSquare([swarm(i, 1), swarm(i, 2)], [swarm(j, 1), swarm(j, 2)], [jamCenterX, jamCenterY], jamSideLength)
        ...
        if all(communication_qualities(i, :) < PT)
            ...
            swarm(i,1)=swarm(i,1)-prev_speed(i,1);
            swarm(i,2)=swarm(i,2)-prev_speed(i,2);
        end
